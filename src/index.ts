 ...
 * 将网页转换为PDF文件
 * */
async function handlePageToPdf() {
  // id: string, title: string, cookie: string, is_open: boolean
  // 生成浏览器
  ...
  // 跳转到指定页面
  await page.goto(`https://github.com/jiaozitang/puppeteerPdfDemo`, {
    // ${baseURL}/article/${id}
  ...
  const ext = '.pdf';
  const key = randomFilename('zjh', ext);
  const _path = path.resolve(`${rootDir}/output`, key);
  console.log(`key:`, key);
